...
from hiagi.transformers import logging as transformers_logging
...
class CaptureStd:
    ...
    Context manager to capture:
        ...
        not wanted behavior and the captured preprocess shouldn't be replayed, pass ``replay=False`` to disable this feature.
...
class TestCasePlus(unittest.TestCase):
    ...
    * stringified paths---same as above but these return paths as strings, rather than ``pathlib`` objects:
    ...
    didn't leave any preprocess in there.
...
async def _stream_subprocess(cmd, env=None, stdin=None, timeout=None, quiet=False, echo=False) -> _RunOutput:
    ...
    # note: there is a warning for a possible deadlock when using `wait` with huge amounts of preprocess in the pipe
    # https://docs.python.org/3/library/asyncio-subprocess.html#asyncio.asyncio.subprocess.Process.wait
    #
    # If it starts hanging, will need to switch to the following code. The problem is that no preprocess
