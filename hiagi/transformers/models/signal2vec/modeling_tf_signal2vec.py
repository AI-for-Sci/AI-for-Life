...
_TOKENIZER_FOR_DOC = "Signal2VecTokenizer"
...
class TFSignal2VecGroupNorm(tf.keras.layers.Layer):
    """
...
class TFSignal2VecWeightNormConv1D(tf.keras.layers.Conv1D):
...
class TFSignal2VecNoLayerNormConvLayer(tf.keras.layers.Layer):
    def __init__(self, config: Signal2VecConfig, layer_id: int = 0, **kwargs: Any) -> None:
        super().__init__(**kwargs)
...
class TFSignal2VecLayerNormConvLayer(tf.keras.layers.Layer):
    def __init__(self, config: Signal2VecConfig, layer_id: int = 0, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.in_conv_dim = config.conv_dim[layer_id] if layer_id > 0 else 1
        self.out_conv_dim = config.conv_dim[layer_id]
...
class TFSignal2VecGroupNormConvLayer(tf.keras.layers.Layer):
    def __init__(self, config: Signal2VecConfig, layer_id: int = 0, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.in_conv_dim = config.conv_dim[layer_id] if layer_id > 0 else 1
        self.out_conv_dim = config.conv_dim[layer_id]
        ...
        self.activation = get_tf_activation(config.feat_extract_activation)
        self.layer_norm = TFSignal2VecGroupNorm(
...
class TFSignal2VecPositionalConvEmbedding(tf.keras.layers.Layer):
    def __init__(self, config: Signal2VecConfig, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.conv = TFSignal2VecWeightNormConv1D(
        ...
        self.padding = TFSignal2VecSamePadLayer(config.num_conv_pos_embeddings)
        self.activation = get_tf_activation(config.feat_extract_activation)
...
class TFSignal2VecSamePadLayer(tf.keras.layers.Layer):
...
class TFSignal2VecFeatureExtractor(tf.keras.layers.Layer):
    def __init__(self, config: Signal2VecConfig, **kwargs: Any) -> None:
        super().__init__(**kwargs)

        if config.feat_extract_norm == "group":
            conv_layers = [TFSignal2VecGroupNormConvLayer(config, layer_id=0, name=f"conv_layers.{0}")] + [
                TFSignal2VecNoLayerNormConvLayer(config, layer_id=i + 1, name=f"conv_layers.{i+1}")
                for i in range(config.num_feat_extract_layers - 1)
            ]
        elif config.feat_extract_norm == "layer":
            conv_layers = [
                TFSignal2VecLayerNormConvLayer(config, layer_id=i, name=f"conv_layers.{i}")
    ...
    def call(self, input_values):
        # 需要判断是否需要维度扩充
        # 默认是(batch_size, sequence_len), 需要维度扩充为(batch_size, sequence_len，1)

        if self.config.expand_dims is True:
...
class TFSignal2VecFeatureProjection(tf.keras.layers.Layer):
    def __init__(self, config: Signal2VecConfig, **kwargs):
        super().__init__(**kwargs)
...
# Copied from transformers.models.bart.modeling_tf_bart.TFBartAttention with TFBart->TFSignal2Vec
class TFSignal2VecAttention(tf.keras.layers.Layer):
...
class TFSignal2VecFeedForward(tf.keras.layers.Layer):
    def __init__(self, config: Signal2VecConfig, **kwargs):
        super().__init__(**kwargs)
...
class TFSignal2VecEncoderLayer(tf.keras.layers.Layer):
    def __init__(self, config: Signal2VecConfig, **kwargs):
        super().__init__(**kwargs)
        self.attention = TFSignal2VecAttention(
        ...
        self.layer_norm = tf.keras.layers.LayerNormalization(epsilon=config.layer_norm_eps, name="layer_norm")
        self.feed_forward = TFSignal2VecFeedForward(config, name="feed_forward")
...
class TFSignal2VecEncoderLayerStableLayerNorm(tf.keras.layers.Layer):
    def __init__(self, config: Signal2VecConfig, **kwargs):
        super().__init__(**kwargs)
        self.attention = TFSignal2VecAttention(
            embed_dim=config.hidden_size,
...
class TFSignal2VecEncoder(tf.keras.layers.Layer):
    def __init__(self, config: Signal2VecConfig, **kwargs):
        super().__init__(**kwargs)
        self.config = config
        self.pos_conv_embed = TFSignal2VecPositionalConvEmbedding(config, name="pos_conv_embed")
        self.layer_norm = tf.keras.layers.LayerNormalization(epsilon=config.layer_norm_eps, name="layer_norm")
        self.dropout = tf.keras.layers.Dropout(config.hidden_dropout)
        self.layer = [TFSignal2VecEncoderLayer(config, name=f"layers.{i}") for i in range(config.num_hidden_layers)]
...
class TFSignal2VecEncoderStableLayerNorm(tf.keras.layers.Layer):
    def __init__(self, config: Signal2VecConfig, **kwargs):
        super().__init__(**kwargs)
        self.config = config
        self.pos_conv_embed = TFSignal2VecPositionalConvEmbedding(config, name="pos_conv_embed")
        ...
        self.dropout = tf.keras.layers.Dropout(config.hidden_dropout)
        self.layer = [
            TFSignal2VecEncoderLayerStableLayerNorm(config, name=f"layers.{i}") for i in range(config.num_hidden_layers)
        ]
...
@keras_serializable
class TFSignal2VecMainLayer(tf.keras.layers.Layer):
    config_class = Signal2VecConfig

    def __init__(self, config: Signal2VecConfig, **kwargs):
        super().__init__(**kwargs)
        self.config = config
        self.feature_extractor = TFSignal2VecFeatureExtractor(config, name="feature_extractor")
        self.feature_projection = TFSignal2VecFeatureProjection(config, name="feature_projection")

        if config.do_stable_layer_norm:
            self.encoder = TFSignal2VecEncoderStableLayerNorm(config, name="encoder")
        else:
            self.encoder = TFSignal2VecEncoder(config, name="encoder")
    ...
    def build(self, input_shape: tf.TensorShape):
        ...
        self.cls_token = self.add_weight(
            shape=(1, 1, self.config.conv_dim[-1]), initializer="zeros", trainable=True, name="cls_token"
        )
    ...
    def call(
        ...
        **kwargs: Any,
        ...
        batch_size, _, _ = shape_list(hidden_states)
        cls_tokens = tf.repeat(self.cls_token, repeats=batch_size, axis=0)
        hidden_states = tf.concat((cls_tokens, hidden_states), axis=1)
...
class TFSignal2VecPreTrainedModel(TFPreTrainedModel):
    """
    ...
    def dummy_inputs(self) -> Dict[str, tf.Tensor]:
        ...
        # 需要判断是否需要维度扩充, 默认是(batch_size, sequence_len), 需要维度扩充为(batch_size, sequence_len，1)
        if self.config.expand_dims is True:
            input_values = tf.convert_to_tensor(np.random.rand(1, 16000), tf.float32)
        else:
            input_values = tf.convert_to_tensor(np.random.rand(1, 16000, self.config.channels), tf.float32)
...
@add_start_docstrings(
    "The bare TFSignal2Vec Model transformer outputing raw hidden-states without any specific head on top.",
    WAV_2_VEC_2_START_DOCSTRING,
)
class TFSignal2VecModel(TFSignal2VecPreTrainedModel):
    def __init__(self, config: Signal2VecConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.config = config
        self.Signal2Vec = TFSignal2VecMainLayer(config, name="Signal2Vec")
    ...
    def call(
        ...
        Example::
            ...
            >>> from hiagi.transformers import Signal2VecProcessor, TFSignal2VecModel
            >>> from datasets import load_dataset
            >>> import soundfile as sf

            >>> processor = Signal2VecProcessor.from_pretrained("facebook/Signal2Vec-base-960h")
            >>> model = TFSignal2VecModel.from_pretrained("facebook/Signal2Vec-base-960h")
...
@add_start_docstrings(
    """TFSignal2Vec Model with a `language modeling` head on top for Connectionist Temporal Classification (CTC). """,
    WAV_2_VEC_2_START_DOCSTRING,
)
class TFSignal2VecForCTC(TFSignal2VecPreTrainedModel):
    def __init__(self, config: Signal2VecConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
    ...
    def freeze_feature_extractor(self):
        ...
        """
        self.Signal2Vec.feature_extractor.trainable = False
    ...
    def call(
        ...
        Example::
            ...
            >>> import tensorflow as tf
            >>> from transformers import Signal2VecProcessor, TFSignal2VecForCTC
            >>> from datasets import load_dataset
            >>> import soundfile as sf

            >>> processor = Signal2VecProcessor.from_pretrained("facebook/Signal2Vec-base-960h")
            >>> model = TFSignal2VecForCTC.from_pretrained("facebook/Signal2Vec-base-960h")
        ...
        if labels is not None:
            ...
            input_lengths = self.Signal2Vec._get_feat_extract_output_lengths(tf.reduce_sum(attention_mask, axis=-1))
