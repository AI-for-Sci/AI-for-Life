...
from hiagi.transformers import MarianConfig, MarianMTModel, MarianTokenizer
...
ROM_GROUP = (
    ...
    "+nap+scn+vec+sc_eeg_fpz_cz+ro+la"
)
GROUPS = [
...
GROUP_TO_OPUS_NAME = {
    ...
    "+pt_PT+gl+lad+an+mwl+it+it_IT+co+nap+scn+vec+sc_eeg_fpz_cz+ro+la",
    ...
    "+es_EC+es_ES+es_GT+es_HN+es_MX+es_NI+es_PA+es_PE+es_PR+es_SV+es_UY+es_VE+pt+pt_br+pt_BR"
    "+pt_PT+gl+lad+an+mwl+it+it_IT+co+nap+scn+vec+sc_eeg_fpz_cz+ro+la-en",
...
def make_registry(repo_path="Opus-MT-train/models"):
    ...
    return [(k, v["pre-preprocess"], v["download"], v["download"][:-4] + ".test.txt") for k, v in results.items()]
...
def _parse_readme(lns):
    ...
    for ln in [x.strip() for x in lns]:
        ...
        for k in ["download", "dataset", "models", "model", "pre-preprocess"]:
            if ln.startswith(k):
                break
        else:
            continue
        if k in ["dataset", "model", "pre-preprocess"]:
            splat = ln.split(":")
            _, v = splat
            subres[k] = v
        elif k == "download":
