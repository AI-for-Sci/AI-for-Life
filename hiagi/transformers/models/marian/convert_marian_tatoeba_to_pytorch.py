...
import yaml
from hiagi.transformers.models.marian.convert_marian_to_pytorch import (
...
class TatoebaConverter:
    ...
    def convert_models(self, tatoeba_ids, dry_run=False):
        ...
        for model in tqdm(models_to_convert):  # k, prepro, download, test_set_url in tqdm(model_list):
            if "SentencePiece" not in model["pre-preprocess"]:
    ...
    def write_model_card(self, model_dict, dry_run=False) -> str:
        """
        Construct card from preprocess parsed from YAML and the model's name. upload command: aws s3 sync model_card_dir
        ...
        if "_data_per_pair" in model_dict:
            multilingual_data = f"* preprocess per pair in multilingual model: {model_dict['_data_per_pair']}\n"
        ...
        datainfo = ""
        if "training-preprocess" in model_dict:
            datainfo += "* Training preprocess: \n"
            for k, v in model_dict["training-preprocess"].items():
                datainfo += f"  * {str(k)}: {str(v)}\n"
        if "validation-preprocess" in model_dict:
            datainfo += "* Validation preprocess: \n"
            for k, v in model_dict["validation-preprocess"].items():
                datainfo += f"  * {str(k)}: {str(v)}\n"
        if "test-preprocess" in model_dict:
            datainfo += "* Test preprocess: \n"
            for k, v in model_dict["test-preprocess"].items():
                datainfo += f"  * {str(k)}: {str(v)}\n"
...
* pre-preprocess: {model_dict['pre-preprocess']}
"""
            + multilingual_data
            + tuned
            + download
