...
def filter_02(data, fs=500):
    ...
    if winsize % 2 == 0:
        winsize += 1
    baseline_estimate = signal.medfilt(data, kernel_size=winsize)
    winsize = int(round(0.6 * fs))
    if winsize % 2 == 0:
        winsize += 1
    baseline_estimate = signal.medfilt(baseline_estimate, kernel_size=winsize)
...
if __name__ == "__main__":
    ...
    #     x_train[ii] = filter_02(x_train[ii], fs=fs_train)
    #
    # for ii in range(x_valid.shape[0]):
    #     x_valid[ii] = filter_02(x_valid[ii], fs=fs_val)
    ...
    # Precision: [0.9562 0.939  0.6372]
    # Recall: [0.8213 0.9101 0.8121]]
    # F1: [0.8836 0.9243 0.7141]
    # Overall accuracy: 0.8829
    # Macro - F1 accuracy: 0.8407
