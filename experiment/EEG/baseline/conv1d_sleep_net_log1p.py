...
if __name__ == "__main__":
    ...
    #     x_train[ii] = filter(x_train[ii], fs=fs_train)
    #
    # for ii in range(x_valid.shape[0]):
    #     x_valid[ii] = filter(x_valid[ii], fs=fs_val)

    x_train = np.reshape(x_train, (x_train.shape[0], x_train.shape[1], 1))
    x_valid = np.reshape(x_valid, (x_valid.shape[0], x_valid.shape[1], 1))

    # x_train = tf.math.log1p(x_train)
    ...
    # Precision: [0.9464 0.3976 0.9267 0.6525 0.6032]
    # Recall: [0.7743 0.1642 0.8691 0.99   0.8613]
    # F1: [0.8517 0.2324 0.897  0.7866 0.7095]
    # Overall accuracy: 0.8034
    # Macro - F1 accuracy: 0.6954
